...
def gen_tinygemm_fp8_module() -> JitSpec:
    """Generate JIT spec for tinygemm_fp8 kernel (Blackwell+ FP8 small GEMM with bias)."""
    nvcc_flags = current_compilation_context.get_nvcc_flags_list(
        supported_major_versions=[10, 11, 12]
    )
    return gen_jit_spec(
        "tinygemm_fp8",
        [jit_env.FLASHINFER_CSRC_DIR / "tinygemm_fp8.cu"],
        extra_cuda_cflags=nvcc_flags,
    )
