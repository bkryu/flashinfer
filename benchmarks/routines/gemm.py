...
def run_gemm_test(args):
    ...
    elif args.routine == "tinygemm_fp8":
        return testTinygemmFp8(args)
    else:
...
def testTinygemmFp8(args):
    """
    Test tinygemm_fp8 API.

    This test:
    1. Generates random input tensors
    2. Quantizes them to FP8 E4M3
    3. Runs tinygemm_fp8
    4. Runs reference check
    5. Measures performance metrics (TFLOPS, TB/sec)
    ...
    """
    if args.verbose >= 1:
        print("[INFO] Running testTinygemmFp8")
        print(f"[INFO] FlashInfer version: {flashinfer.__version__}")
    ...
    mat2_dtype = dtype_str_to_torch_dtype(args.mat2_dtype)
    out_dtype = dtype_str_to_torch_dtype(args.out_dtype)
    if input_dtype != torch.float8_e4m3fn or mat2_dtype != torch.float8_e4m3fn:
        raise ValueError(
            "tinygemm_fp8 only supports fp8_e4m3 input and weight tensors."
        )
    if out_dtype not in [torch.bfloat16, torch.float16]:
        raise ValueError("tinygemm_fp8 only supports bfloat16 or float16 outputs.")
    ...
    input_f32 = torch.randn([m, k], device=device, dtype=torch.float32)
    weight_nk_f32 = torch.randn([n, k], device=device, dtype=torch.float32)
    input_fp8, input_inv_s = to_float8(input_f32, dtype=input_dtype)
    weight_row_major_fp8, weight_inv_s = to_float8(weight_nk_f32, dtype=mat2_dtype)
    weight_fp8 = weight_row_major_fp8.t()
    ...
    if args.verbose >= 2:
        ...
        print(f"[VVERBOSE] {weight_fp8.dtype = }")
        print(f"[VVERBOSE] {input_inv_s = }")
        print(f"[VVERBOSE] {weight_inv_s = }")
        if bias is not None:
            print(f"[VVERBOSE] {bias.shape = }")
            print(f"[VVERBOSE] {bias.dtype = }")
        print(f"[VVERBOSE] {use_pdl = }")
    ...
    has_reference_output = False
    if run_refcheck:
        reference_output = (input_fp8.float() @ weight_fp8.float()) * (
            input_inv_s.float() * weight_inv_s.float()
        )
        if bias is not None:
            reference_output = reference_output + bias.float()
    ...
    for backend in backends:
        if len(backend_times[backend]) > 0:
            ...
            problem_bytes = (
                ...
                + n * k * mat2_dtype.itemsize
                + m * n * out_dtype.itemsize
                + 2 * torch.float32.itemsize
            )
            if use_bias:
                problem_bytes += n * out_dtype.itemsize
            tflops = problem_flops / (10**9 * median_time)
            tb_per_sec = problem_bytes / (10**9 * median_time)
            print_perf_metrics(backend, median_time, std_time, tflops, tb_per_sec)
